package com.paremus.brain.iot.example.robotB.impl;
...
@SmartBehaviourDefinition(consumed = {writeGOTO.class,Cancel.class, PickCart.class,PlaceCART.class,QueryStateB.class,CheckMarkerB.class })
public class ComponentImpl implements SmartBehaviour<BrainIoTEvent>{
	...
	public static QueryStateB querySate;
	public static CheckMarkerB checkmaker;
	
	
    public static CheckValueReturnB checkreturnedvalue;
    
    public static QueryStateValueReturnB queryreturnedvalue;
				...
				if (event instanceof PickCart) {
					...
					if (event instanceof PlaceCART) {
						...
						synchronized (this) {
							...
							placeCART(2, plcacart.cart);

						}
						
					}else {
						if (event instanceof QueryStateB) {
							querySate = (QueryStateB) event;
							synchronized (this) {
								if(querySate.robotid==2) {
									queryreturnedvalue =new QueryStateValueReturnB ();
									queryreturnedvalue.value =queryState(2, querySate.mission);
							...
							if (event instanceof CheckMarkerB) {
								checkmaker = (CheckMarkerB) event;
								synchronized (this) {
									if(checkmaker.robotid==2) {
										checkreturnedvalue =new CheckValueReturnB();
										checkreturnedvalue.value=checkMarkers(2,5);
