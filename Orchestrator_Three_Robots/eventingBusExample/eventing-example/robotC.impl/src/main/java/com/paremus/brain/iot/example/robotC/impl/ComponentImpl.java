package com.paremus.brain.iot.example.robotC.impl;
...
@SmartBehaviourDefinition(consumed = {writeGOTO.class,Cancel.class, PickCart.class,PlaceCART.class,QueryStateC.class,CheckMarkerC.class })
public class ComponentImpl implements SmartBehaviour<BrainIoTEvent>{
    
	static String IP="10.8.0.50";
	...
	public static QueryStateC querySate;
	public static CheckMarkerC checkmaker;
	
	
    public static CheckValueReturnC checkreturnedvalue;
    
    public static QueryStateValueReturnC queryreturnedvalue;
				...
				if (event instanceof PickCart) {
					...
					if (event instanceof PlaceCART) {
						...
						synchronized (this) {
							...
							placeCART(3, plcacart.cart);

						}
						
					}else {
						if (event instanceof QueryStateC) {
							querySate = (QueryStateC) event;
							synchronized (this) {
								if(querySate.robotid==3) {
									queryreturnedvalue =new QueryStateValueReturnC();
									queryreturnedvalue.value =queryState(3, querySate.mission);
							...
							if (event instanceof CheckMarkerC) {
								checkmaker = (CheckMarkerC) event;
								synchronized (this) {
									if(checkmaker.robotid==3) {
										checkreturnedvalue =new CheckValueReturnC ();
										checkreturnedvalue.value=checkMarkers(3,5);
